...
def relu_backward(dout, cache):
    ...
    dx, x = None, cache
    dx = np.zeros(x.shape)
    arg_nonzero = np.where(x>0)
    ###########################################################################
    # TODO: Implement the ReLU backward pass.                                 #
    ###########################################################################
    dx[arg_nonzero] = dout[arg_nonzero]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def batchnorm_backward(dout, cache):
    ...
    dxm2 = 2*xmean*dsq
    dx1 = dxm1 + dxm2
    dm = -1 * np.sum(dxm1 + dxm2, axis=0)
    dx2 = (-dm*np.ones((N,D)))/N
    '''
    #step9
    ...
    #dbeta=np.sum(dout, axis=0)      # правильное
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma=np.sum(dout*xhat, axis=0) 
    dbeta=np.sum(dout, axis=0)
    dx = (1./N) * gamma * ivar * (N * dout - np.sum(dout, axis=0) - xmu * (var + eps)**(-1)
                                 * np.sum(dout*xmu, axis=0))
     
    #dgamma=np.sum(xmu*ivar*dout, axis=0)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = (np.random.rand(*x.shape) < (1-p)) / (1-p) # first dropout mask. Notice /p!
        out = x * mask
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x_pad = np.pad(x,((0,0),(0,0),(pad,pad),(pad,pad)),'constant', constant_values = 0)    #добавляем pad
    
    for n in range(N):   # цикл по изображениям
        for f in range(F):       # цикл по фильтрам
            b0 = b[f] 
            for i in range(H_out):           # цикл по строкам выходной матрицы
                for j in range(W_out):       # цикл по столбцам выходной матрицы
                    s = 0
                    for c in range(C):       # цикл по каналам изображения
                        h_begin =(stride*i)
                        h_end = (HH+stride*i)
                        w_begin = (stride*j)
                        w_end = (WW+stride*j)
                        x0 = x_pad[n][c][h_begin:h_end,w_begin:w_end]
                        w0 = w[f][c]
                        s += np.sum(x0*w0)
                    out[n][f][i][j] = s + b0 
...
def conv_backward_naive(dout, cache):
    ...
    dx = np.zeros((N, C, H, W))
    dx_pad = np.zeros((H+(2*pad), W+(2*pad)))
    ...
    for c in range(C):   # цикл по каналам изображения
        for f in range(F):       # цикл по фильтрам
            for i in range(HH):           # цикл по строкам фильтра
                for j in range(WW):       # цикл по столбцам фильтра
                    s = 0
                    for n in range(N):    # цикл по изображениям
                        h_begin =(stride*i)
                        h_end = (H_out+stride*i)
                        w_begin = (stride*j)
                        w_end = (W_out+stride*j)
                        #print('{}, {}'.format(i, j))
                        #print('{}:{} {}:{}'.format(h_begin,h_end,w_begin,w_end))
                        x0 = x_pad[n][c][h_begin:h_end,w_begin:w_end]
                        #w0 = w[f][c]
                        dout0 = dout[n][f]
                        #print('x0', x0.shape)
                        #print('dout0', dout0.shape)
                        s += np.sum(x0*dout0)
    ...
    for n in range(N):   # цикл по изображениям
        for c in range(C):        # цикл по каналам изображения
            dx_pad = np.zeros((H+(2*pad), W+(2*pad)))
            for i in range(H_out):           # цикл по строкам входной матрицы
                for j in range(W_out):       # цикл по столбцам входной матрицы
                    for f in range(F):      # цикл по фильтрам
                        h_begin =(stride*i)
                        h_end = (HH+stride*i)
                        w_begin = (stride*j)
                        w_end = (WW+stride*j)
                        #print('n {}, f {}, i {}, j {}'.format(n,f,i,j))
                        #dout0 = dout[n][f][i][j]
                        dout0 = dout[n,f,i,j]
                        w0 = w[f][c]
                        dx_pad[h_begin:h_end,w_begin:w_end] += w0*dout0           
